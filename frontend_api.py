...
if os.getenv("CLOUD_DEPLOYMENT") == "true" or os.getenv(
    "K_SERVICE"
...
"""
        ...
        confidence_level = (
            "high certainty"
            if confidence > 70
            else "moderate certainty" if confidence > 50 else "low certainty"
        )
        signal_direction = (
            "positive" if signal > 0 else "negative" if signal < 0 else "neutral"
        )
...
"""
        ...
        if neutral_agents:
            neutral_text = (
                f"{', '.join(neutral_agents)} analysis shows neutral positioning."
            )
        else:
            neutral_text = ""

        signal_summary = " ".join(
            filter(None, [positive_text, negative_text, neutral_text])
        )
        ...
        if fund_data.get("directional_signal", 0) > 0.3:
            insights.append(
                f"- **Strong fundamentals**: {fund_data.get('summary', 'Positive financial indicators')}"
            )
        elif fund_data.get("directional_signal", 0) < -0.3:
            insights.append(
                f"- **Fundamental concerns**: {fund_data.get('summary', 'Valuation or financial challenges')}"
            )

        # Technical insights
        if tech_data.get("directional_signal", 0) > 0.3:
            insights.append(
                f"- **Bullish technicals**: {tech_data.get('summary', 'Positive momentum indicators')}"
            )
        elif tech_data.get("directional_signal", 0) < -0.3:
            insights.append(
                f"- **Bearish technicals**: {tech_data.get('summary', 'Negative price action')}"
            )

        # Sentiment insights
        news_count = sent_data.get("key_metrics", {}).get("news_count", 0)
        if news_count > 0:
            sentiment_tone = sent_data.get("key_metrics", {}).get(
                "sentiment", "neutral"
            )
            insights.append(
                f"- **Market sentiment**: {news_count} recent articles show {sentiment_tone} tone"
            )
        ...
        if macro_data.get("directional_signal", 0) != 0:
            ...
            insights.append(
                f"- **Economic environment**: {regime.title()} regime with Fed rate at {fed_rate}"
            )
        ...
        if recommendation == "BUY":
            next_steps = (
                """1. Consider establishing a position sized according to your portfolio allocation strategy
2. Monitor the positive factors identified (especially """
                + ", ".join(positive_agents[:2])
                + """) for any changes
...
4. Review quarterly earnings and news for confirmation of the thesis"""
            ...
            next_steps = (
                """1. Evaluate your current position and consider reducing exposure gradually
2. Watch for any improvement in """
                + ", ".join(negative_agents[:2])
                + """ indicators
...
4. Maintain vigilance on news that could change the outlook"""
            ...
            next_steps = (
                """1. Place {ticker} on your watchlist for future opportunities
2. Wait for clearer signals from """
                + ", ".join(neutral_agents[:2])
                + """ analysis
...
async def analyze_stock(request: AnalyzeRequest):
    ...
    try:
        ...
        result = orch.analyze_stock(
            ticker=request.ticker.upper(), horizon=request.horizon, verbose=False
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to analyze {request.ticker}: {str(e)}"
        )
