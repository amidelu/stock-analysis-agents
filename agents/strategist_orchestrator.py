...
class StrategistOrchestrator:
    ...
    def _create_remote_agent(self, base_url: str, name: str) -> RemoteA2aAgent:
        ...
        try:
            ...
            raise RuntimeError(
                f"Agent {name} is not reachable at {base_url}. Make sure all agents are running: bash scripts/start_all_agents.sh"
            )

    async def _run_agent_and_get_response(
        self, agent: RemoteA2aAgent, prompt: str
    ) -> str:
    ...
    def __init__(self):
        ...
        self.fundamental_agent = self._create_remote_agent(
            "http://localhost:8001", "fundamental_analyst"
        )
        print("   ✅ Connected to Fundamental Analyst")

        self.technical_agent = self._create_remote_agent(
            "http://localhost:8002", "technical_analyst"
        )
        print("   ✅ Connected to Technical Analyst")

        self.sentiment_agent = self._create_remote_agent(
            "http://localhost:8003", "news_sentiment_analyst"
        )
        print("   ✅ Connected to Sentiment Analyst")

        self.macro_agent = self._create_remote_agent(
            "http://localhost:8004", "macro_analyst"
        )
        print("   ✅ Connected to Macro Analyst")

        self.regulatory_agent = self._create_remote_agent(
            "http://localhost:8005", "regulatory_analyst"
        )
        print("   ✅ Connected to Regulatory Analyst")

        self.predictor_agent = self._create_remote_agent(
            "http://localhost:8006", "predictor_agent"
        )
    ...
    async def analyze_stock_async(
        self, ticker: str, horizon: str = "next_quarter", verbose: bool = False
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            fundamental_report = (
                ...
                else {"error": str(analysis_results[0])}
            )
            technical_report = (
                analysis_results[1]
                if not isinstance(analysis_results[1], Exception)
                else {"error": str(analysis_results[1])}
            )
            sentiment_report = (
                analysis_results[2]
                if not isinstance(analysis_results[2], Exception)
                else {"error": str(analysis_results[2])}
            )
            macro_report = (
                analysis_results[3]
                if not isinstance(analysis_results[3], Exception)
                else {"error": str(analysis_results[3])}
            )
            regulatory_report = (
                analysis_results[4]
                if not isinstance(analysis_results[4], Exception)
                else {"error": str(analysis_results[4])}
            )
            ...
            prediction_response_text = await self._run_agent_and_get_response(
                self.predictor_agent, prediction_prompt
            )
    ...
    async def _call_agent_async(
        self, agent_type: str, ticker: str, verbose: bool
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"error": str(e), "agent": agent_type, "ticker": ticker}
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Stock Prediction System - The Strategist"
    )
    parser.add_argument(
        "--ticker", "-t", required=True, help="Stock ticker symbol (e.g., GOOGL)"
    )
    parser.add_argument(
        "--horizon", "-H", default="next_quarter", help="Prediction horizon"
    )
    parser.add_argument(
        "--verbose", "-v", action="store_true", help="Show intermediate outputs"
    )
    ...
    result = strategist.analyze_stock(
        ticker=args.ticker.upper(), horizon=args.horizon, verbose=args.verbose
    )
